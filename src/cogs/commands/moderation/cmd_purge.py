...
def cog_creator(servers: List[int]):
    class Purge(BaseCog):
        ...
        @BaseCog.cslash_command(description="Purges messages by default. If 'member' is provided, it will purge messages by that member only", guild_ids=servers)
        async def purge(self, ctx: ApplicationContext,amount: Option(int), member: Option(discord.Member) = None):
            if not self.check_perms(ctx, {"manage_messages": True}, member, True):
