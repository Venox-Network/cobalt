...
def cog_creator(servers: List[int]):
    class All_Warns(BaseCog):
        ...
        @BaseCog.cslash_command(description="Warns a member", guild_ids=servers)
        async def warn(self, ctx: ApplicationContext, member: Option(discord.Member), reason: Option(str) = None):
        ...
        @BaseCog.cslash_command(description="Gets all the warn info of a member", guild_ids=servers)
        async def warns(self, ctx: ApplicationContext, member: Option(discord.Member)):
            ...
            try:
                ...
                await ctx.respond(f"Could not interract with database `warns`. With error {e}.", ephemeral=True)
                return
