...
def cog_creator(servers: List[int]):
    class Mute(BaseCog):
        ...
        @BaseCog.cslash_command(description="Mutes a member using the timeout function", guild_ids=servers)
        async def mute(self, ctx: ApplicationContext, member: Option(discord.Member), time: Option(str), reason: Option(str) = None):
            ...
            try:
                ...
                time_unix = int((datetime.now() + timedelta(seconds=time_readable)).timestamp())
