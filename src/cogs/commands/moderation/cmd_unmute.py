...
def cog_creator(servers: List[int]):
    class Unmute(BaseCog):
        ...
        @BaseCog.cslash_command(description="Unmutes a member using the timeout function", guild_ids=servers)
        async def unmute(self, ctx: ApplicationContext, member: Option(discord.Member), reason: Option(str) = None):
