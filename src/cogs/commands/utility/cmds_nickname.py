...
def cog_creator(servers: List[int]):
    class NickNameCog(BaseCog):
        ...
        @BaseCog.cslash_command(description="Marks name as the nickname to be set for violators", guild_ids=servers)
        async def setup_nickname(self, ctx: ApplicationContext, new_nickname: Option(str)):
        ...
        @BaseCog.cslash_command(description="Adds a nickname to the blacklist", guild_ids=servers)
        async def blacklist_name(self, ctx: ApplicationContext, name: Option(str)):
            if not self.check_perms(ctx, {"administrator": True}):
                await ctx.respond("Sorry, you cannot use this command.", ephemeral=True)
                return
            ...
            try:
                ...
                if result is None:
                    ...
                    await ctx.respond("Nickname added to Blacklist.", ephemeral=True)
                    return

                if name in result["nicknames"]:
                    await ctx.respond("Nickname is already in the blacklist.", ephemeral=True)
                    return
        ...
        @BaseCog.cslash_command(description="Removes a nickname from the blacklist", guild_ids=servers)
        async def remove_blacklist_name(self, ctx: ApplicationContext, name: Option(str)):
            if not self.check_perms(ctx, {"administrator": True}):
                await ctx.respond("Sorry, you cannot use this command.", ephemeral=True)
                return
            ...
            try:
                ...
                await ctx.respond(f"Could not interract with database `nicknames`. With error: {e}", ephemeral=True)

        @BaseCog.cslash_command(description="List all the blacklisted nicknames on this server", guild_ids=servers)
        async def blacklist_nick_list(self, ctx: ApplicationContext, hide: Option(bool) = True):
            if not self.check_perms(ctx, {"administrator": True}):
                await ctx.respond("Sorry, you cannot use this command.", ephemeral=True)
                return
            ...
            try:
                ...
                result = await self.nick_names.find_one({"guild_id": ctx.guild.id})
                if result is None:
                    await ctx.respond("No Nicknames blacklisted on this server.",ephemeral=hide)
                    return
                await ctx.respond("Blacklisted Nicknames:\n" + "\n".join(result["nicknames"]))
            except Exception as e:
                await ctx.respond(f"Could not interract with database `nicknames`. With error: `{e}`", ephemeral=hide)
