...
def cog_creator(servers: List[int]):
    async def check_voice(ctx: ApplicationContext):
    ...
    class MusicCog(BaseCog):
        ...
        async def on_wavelink_track_end(self, voice_chat: wavelink.Player, track: wavelink.Track):
            ...
            try:
                ...
                if getattr(voice_chat, "text_channel", None) is not None:
                    await voice_chat.text_channel.send(f"Now playing: `{next_music.title}`")
            except Exception:
                voice_chat.loop = False
                voice_chat.text_channel = None
                await voice_chat.disconnect()

        @BaseCog.cslash_command(description="Play music", guild_ids=servers)
        async def play(self, ctx: ApplicationContext, search: Option(str, "Song Name")):
            try:
        ...
        @BaseCog.cslash_command(description="Toggle Music b/w Playing and Paused", guild_ids=servers)
        async def toggle_music(self, ctx: ApplicationContext):
            if not ctx.voice_client:
        ...
        @BaseCog.cslash_command(description="Stops Music and clears queue", guild_ids=servers)
        async def stop_music(self, ctx: ApplicationContext):
            if not ctx.voice_client:
                await ctx.respond("There is no music being played on this server.")
                return
        ...
        @BaseCog.cslash_command(description="Skip current song", guild_ids=servers)
        async def skip_music(self, ctx: ApplicationContext):
            if not ctx.voice_client:
                await ctx.respond("There is no music being played on this server.")
                return
        ...
        @BaseCog.cslash_command(description="Disconnect from voice channel", guild_ids=servers)
        async def voice_leave(self, ctx: ApplicationContext):
            if not ctx.voice_client:
                await ctx.respond("There is no music being played on this server.")
                return
        ...
        @BaseCog.cslash_command(description="Loops over the current music", guild_ids=servers)
        async def loop_music(self,ctx: ApplicationContext):
            if not ctx.voice_client:
                await ctx.respond("There is no music being played on this server.")
                return
        ...
        @BaseCog.cslash_command(description="Show the music queue.", guild_ids=servers)
