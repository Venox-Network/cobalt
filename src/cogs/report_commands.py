    ...
    class ReportCog(BaseCog):
        ...
        async def on_guild_channel_delete(self, channel: discord.TextChannel):
            ...
            try:
                ...
                await self.bot.log_msg(f"Error while deleting channel: `{channel.name}`[{channel.id}] from DB `report_channels`\n\n{str(e)}", True)
        ...
        async def report(
            ...
            if local_report_channel is None:
                await self.bot.log_msg(f"`{ctx.user.name}#{ctx.user.discriminator}` tried to report on `{ctx.guild.name}`, local report channel for `{ctx.guild.name}` not found.")
