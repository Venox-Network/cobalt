...
public class AutoDeleteCmd extends ApplicationCommand {
    ...
    public void listCommand(@NotNull GuildSlashEvent event) {
        final Map<Long, Set<Long>> autoDeletes = bot.oldData.getGuild(event.getGuild()).autoDeletes;

        // Check if empty
        if (autoDeletes.isEmpty()) {
            event.reply(LazyEmoji.NO + " No channels have message auto-deletion enabled").setEphemeral(true).queue();
            return;
        }
        ...
        event.reply(LazyEmoji.YES + " " + builder.substring(0, builder.length() - 2)).setEphemeral(true).queue();
    }
    ...
    public void enableCommand(@NotNull GuildSlashEvent event,
        ...
        if (autoDeletes.containsKey(channel.getIdLong())) {
            event.reply(LazyEmoji.NO + " Message auto-deletion is already enabled for " + channel.getAsMention()).setEphemeral(true).queue();
            return;
        }

        // Enable auto-deletion
        autoDeletes.put(channel.getIdLong(), new HashSet<>());
        event.reply(LazyEmoji.YES + " Message auto-deletion is now enabled for " + channel.getAsMention()).setEphemeral(true).queue();
    }
    ...
    public void disableCommand(@NotNull GuildSlashEvent event,
        ...
        if (!autoDeletes.containsKey(channel.getIdLong())) {
            event.reply(LazyEmoji.NO + " Message auto-deletion is already disabled for " + channel.getAsMention()).setEphemeral(true).queue();
            return;
        }

        // Disable auto-deletion
        autoDeletes.remove(channel.getIdLong());
        event.reply(LazyEmoji.YES + " Message auto-deletion is now disabled for " + channel.getAsMention()).setEphemeral(true).queue();
    }
    ...
    public void bypassListCommand(@NotNull GuildSlashEvent event,
        ...
        if (autoDelete.isEmpty()) {
            event.reply(LazyEmoji.NO + " No roles are in the bypass list for " + channel.getAsMention()).setEphemeral(true).queue();
            return;
        }
    ...
    public void bypassAddCommand(@NotNull GuildSlashEvent event,
        ...
        if (autoDelete.contains(role.getIdLong())) {
            event.reply(LazyEmoji.NO + " " + role.getAsMention() + " is already in the bypass list for " + channel.getAsMention()).setEphemeral(true).queue();
            return;
        }

        // Add role
        autoDelete.add(role.getIdLong());
        event.reply(LazyEmoji.YES + " " + role.getAsMention() + " is now in the bypass list for " + channel.getAsMention()).setEphemeral(true).queue();
    }
    ...
    public void bypassRemoveCommand(@NotNull GuildSlashEvent event,
        ...
        // Check if not added
        if (!autoDelete.contains(role.getIdLong())) {
            event.reply(LazyEmoji.NO + " " + role.getAsMention() + " is not in the bypass list for " + channel.getAsMention()).setEphemeral(true).queue();
            return;
        }

        // Remove role
        autoDelete.remove(role.getIdLong());
        event.reply(LazyEmoji.YES + " " + role.getAsMention() + " is no longer in the bypass list for " + channel.getAsMention()).setEphemeral(true).queue();
    }

    @Nullable
    private Set<Long> getAutoDelete(@NotNull GuildSlashEvent event, @NotNull GuildChannel channel) {
        final Set<Long> autoDelete = bot.oldData.getGuild(event.getGuild()).autoDeletes.get(channel.getIdLong());
        if (autoDelete == null) event.reply(LazyEmoji.NO + " Message auto-deletion is not enabled for " + channel.getAsMention()).setEphemeral(true).queue();
        return autoDelete;
    }
}
