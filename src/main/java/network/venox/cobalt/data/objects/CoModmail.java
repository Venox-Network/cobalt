...
public final class CoModmail extends CoObject {
    ...
    public final long dmStartMessageId;
    public boolean expireWarningSent;
    @Nullable private Long futureStart;
    @Nullable private ScheduledFuture<?> future;

    public CoModmail(@NotNull Cobalt cobalt, long userId, long threadId, long dmStartMessageId, boolean closed, boolean expireWarningSent, @Nullable Long futureStart) {
        this.cobalt = cobalt;
        this.userId = userId;
    ...
    public Map<String, Object> toMap() {
        ...
        map.put("dm-start-message", dmStartMessageId);
        if (expireWarningSent) map.put("expire-warning-sent", true);
        if (futureStart != null) map.put("future-start", futureStart);
    ...
    public boolean isNull() {
        return getUser() == null || getThread() == null;
    }

    @Nullable
    public CacheRestAction<User> getUser() {
        return MiscUtility.handleException(() -> cobalt.jda.retrieveUserById(userId));
    }
    ...
    @Nullable
    private RestAction<PrivateChannel> getPrivateChannel() {
        final CacheRestAction<User> userAction = getUser();
        if (userAction == null) return null;
        return userAction.flatMap(User::openPrivateChannel);
    }

    @Nullable
    public MessageEmbed getUserEmbed(@NotNull String description, long messageId) {
        final CacheRestAction<User> userAction = getUser();
        if (userAction == null) return null;
        final User user = userAction.complete();
        return new EmbedBuilder()
                .setColor(Color.GREEN)
                .setAuthor(user.getAsTag(), AUTHOR_URL + messageId, user.getEffectiveAvatarUrl())
    ...
    public MessageEmbed getModeratorEmbed(@NotNull User moderator, @NotNull String description, long messageId) {
        final String tag = moderator.getAsTag();
        return new EmbedBuilder()
                .setColor(Color.BLUE)
                .setAuthor(tag, AUTHOR_URL + messageId, moderator.getEffectiveAvatarUrl())
                .setDescription(description)
                .build();
    }

    public void close(@Nullable User closer) {
        closed = true;
        final ThreadChannel thread = getThread();
        if (thread != null) thread.retrieveMessageById(thread.getIdLong())
                ...
                .flatMap(msg -> thread.sendMessage("\uD83D\uDD12 " + (closer == null ? "The modmail thread has expired" : "**`" + closer.getAsTag() + "`** has closed the modmail thread") + "!"))
                .flatMap(msg -> thread.getManager().setArchived(true))
                .queue();
        final RestAction<PrivateChannel> privateChannelAction = getPrivateChannel();
        if (privateChannelAction != null) privateChannelAction
                .flatMap(privateChannel -> privateChannel.sendMessage("\uD83D\uDD12 " + (closer == null ? "Your modmail thread has expired" : "**`" + closer.getAsTag() + "`** has closed your modmail thread") + "!")
                        .flatMap(msg -> privateChannel.retrieveMessageById(dmStartMessageId)))
                .flatMap(msg -> msg.editMessageComponents(List.of()))
                .queue();
    }

    public void delete(@NotNull User deleter) {
        cobalt.dataManager.oldData.global.modmails.remove(this);
        final ThreadChannel thread = getThread();
        if (thread != null) thread.delete().queue();
        final RestAction<PrivateChannel> privateChannelAction = getPrivateChannel();
        if (privateChannelAction != null) privateChannelAction
                .flatMap(privateChannel -> privateChannel.sendMessage(LazyEmoji.TRASH + "️ **`" + deleter.getAsTag() + "`** has deleted your modmail thread!")
                        .flatMap(msg -> privateChannel.retrieveMessageById(dmStartMessageId)))
                .flatMap(msg -> msg.editMessageComponents(List.of()))
                .queue();
    }
    ...
    public void checkExpiration() {
        ...
        if (futureStart == null) {
            ...
            if (expireWarningSent) {
                close(null);
                return;
            }

            // Expire warning not yet sent
            scheduleExpireWarning(null);
            return;
        }
        ...
        scheduleExpireWarning(EXPIRE_TIME.toMillis() - timeSinceFutureScheduled);
    }

    public void scheduleExpireWarning(@Nullable Long delay) {
        ...
        future = Cobalt.SCHEDULED_EXECUTOR_SERVICE.schedule(() -> {
            final ThreadChannel thread = getThread();
            final RestAction<PrivateChannel> privateChannelAction = getPrivateChannel();
            if (closed || thread == null || privateChannelAction == null) return;

            // Check if user was the last to talk
            try {
                if (Boolean.TRUE.equals(userTalkedLast().get())) return;
            ...
            future = Cobalt.SCHEDULED_EXECUTOR_SERVICE.schedule(() -> {
                if (!closed) try {
                    if (Boolean.FALSE.equals(userTalkedLast().get())) close(null);
                } catch (final InterruptedException | ExecutionException e) {
                    e.printStackTrace();
                    if (e instanceof InterruptedException) Thread.currentThread().interrupt();
                }
    ...
    private CompletableFuture<Boolean> userTalkedLast() {
        final ThreadChannel thread = getThread();
        ...
        confirmation.addActionRow(
                Components.successButton(buttonEvent -> {
                    ...
                    buttonEvent.deferEdit()
                            .flatMap(hook -> hook.editOriginalComponents(List.of())
                                    .flatMap(msg -> forumChannel.createForumPost(userButton.getAsTag(), MessageCreateData.fromContent("Getting the mods!")))
                                    .flatMap(post -> {
                                        ...
                                        cobalt.dataManager.oldData.global.modmails.add(modmail);
    ...
    @NotNull
    private static Button getCloseButton(@NotNull String name) {
